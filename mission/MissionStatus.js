...
const doLocal = false;
const G_MIN_TOPIC_ITEM_TIME = doLocal ? '2022/11/29' : '2021/03/01';
...
async function handleFetchTopic(page, topic) {
  ...
  const forceCloseUrl = `${getMeteorHost()}/submitForceSetTopicAsHandled`;
  await axios.post(forceCloseUrl, data);
...
async function executeTopicMissionGroup(iterCount) {
  //1.create browser
